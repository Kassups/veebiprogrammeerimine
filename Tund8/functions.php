<?php
	...
	function saveIdea($idea, $color){
		$notice ="";
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		$stmt = $mysqli->prepare("INSERT INTO vpluserideas siin peab minu oma olema (userid, idea, ideacolors)VALUES (?, ?, ?)");
		echo $mysqli->error;
		$stmt->bind_param("iss", $_SESSION["userId"], $idea, $color);
		if($stmt->execute()){
			$notice = "Mõte on talletatud!";
			}	else{
	...
	function listIdeas(){
		$notice ="";
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		//$stmt= $mysqli->prepare("SELECT idea, ideacolor FROM vp1users enda oma peab olema");
		//$stmt= $mysqli->prepare("SELECT idea, ideacolor FROM vp1users enda oma peab olema ORDER BY id DESC");
		$stmt= $mysqli->prepare("SELECT id, idea, ideacolor FROM vp1users enda oma peab olema WHERE userid = ? ORDER BY id DESC");
		echo $mysqli->error;
		...
		while($stmt->fetch()){
			...
			//$notice .= 'style="background-color: ' .$color .'">' .$idea . "</p> \n";
			$notice .= 'style="background-color: ' .$color .'">' .$idea .' | <a href="edituseridea.php?id=' .$id  .'">Toimeta</a>' ."</p> \n";
	...
	function latestIdea(){
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		$stmt = $mysqli->prepare("SELECT idea FROM vpusers see ka minu oma WHERE id =(SELECT MAX(id) FROM vp minu user)");
		echo $mysqli->error;
