<?php
	...
	if (isset ($_POST["signupEmail"])){
		if (empty ($_POST["signupEmail"])){
			...
			$signupEmail = test_intput($_POST["signupEmail"]);
	...
	if (empty($signupFirstNameError) and empty($signupFamilyNameError) and empty($signupBirthDayError) and empty($signupGenderError) and empty($signupEmailError) and empty($signupPasswordError)){
		...
		singUp($signupFirstName, $signupFamilyName, $signupBirthDate, $gender, $signupEmail, $signupPassword);
...
<body>
	...
	<form method="POST">
		...
		<label>Sugu</label>
		<br>
		<input type="radio" name="gender" value="1" <?php if ($gender == "1") {echo 'checked';} ?>><label>Mees</label> <!-- Kõik läbi POSTi on string!!! -->
